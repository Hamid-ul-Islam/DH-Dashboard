...
export default function Product({
  ...
  async function handleDeleteImage(index) {
    ...
    await axios.delete(`/api/upload?img=${updatedImages[index]}`);
              ...
              >
                {images?.map((link, index) => (
                  <div key={link} className="relative group">
                    ...
                    />
                    <div
                      className={`absolute top-0 flex justify-center items-center w-full bg-indigo-200 text-indigo-500 px-1 rounded opacity-90 text-sm ${
